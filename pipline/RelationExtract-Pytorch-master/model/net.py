...
class CNN_test(nn.Module):
	def __init__(self, data_loader, params):
		...
		self.idx_e1=data_loader.e1_idx
		self.idx_e2 = data_loader.e2_idx
		...
		self.covns_rang_e1_e2 = nn.ModuleList([nn.Sequential(nn.Conv1d(in_channels=params.word_emb_dim,
		...
		# output layer
		self.linear_rang = nn.Linear(filter_dim*2, labels_num)
	...
	def forward(self, x):
		...
		# 新加的
		ebed_range_e1_2=self.rang_e1_e2(word_embs)
		out_r = [conv(self.dropout(torch.add(word_embs,ebed_range_e1_2).permute(0,2,1))) for conv in self.covns_rang_e1_e2]
		...
		x=self.linear_rang(x)

		return x

	def rang_e1_e2(self,wordembeding):
		ebed_new=torch.zeros(wordembeding.size())
		for j in range(wordembeding.size(0)):
			low=self.idx_e1[j]-5
			high=self.idx_e2[j]+5
			k=low
			while k<=high:
				ebed_new[j][k]=wordembeding[j][k]
				k=k+1
