...
from tplinker.tplinker import (HandshakingTaggingScheme,
                               DataMaker4Bert,
                               DataMaker4BiLSTM,
                               TPLinkerBert,
                               TPLinkerBiLSTM,
...
sys.path.append('')
from tplinker.config import train_config
...
if config["encoder"] == "BERT":
    ...
    # tokenizer = pytorch_pretrained_bert.BertModel.from_pretrained(config["bert_path"], add_special_tokens=False, do_lower_case=False)
    print('===========================================')
    tokenize = tokenizer.tokenize # 分词
    get_tok2char_span_map = lambda text: \
    tokenizer.encode_plus(text, return_offsets_mapping=True, add_special_tokens=False)["offset_mapping"] # 编码
...
preprocessor = Preprocessor(tokenize_func=tokenize,
                            get_tok2char_span_map_func=get_tok2char_span_map) # 提前先将分词和编码工具传给Preprocessor类
...
if config["encoder"] == "BERT":
    tokenizer = BertTokenizerFast.from_pretrained(config["bert_path"], add_special_tokens=False, do_lower_case=False)
    data_maker = DataMaker4Bert(tokenizer, handshaking_tagger) #传参给DataMaker4Bert类
...
if config["encoder"] == "BERT":
    encoder = AutoModel.from_pretrained(config["bert_path"]) # 采用bert编码
...
def train_step(batch_train_data, optimizer, loss_weights):
    ...
    w_ent, w_rel = loss_weights["ent"], loss_weights["rel"] # 标签 batch_ent_shaking_tag，预测ent_shaking_outputs
...
def train_n_valid(train_dataloader, dev_dataloader, optimizer, scheduler, num_epoch):
    def train(dataloader, ep):
        ...
        for batch_ind, batch_train_data in enumerate(dataloader):
            t_batch = time.time()
            z = (2 * len(rel2id) + 1) # *2表示某种【实体头-实体头】和【实体尾-实体尾】两种矩阵，+1为【实体头-实体尾】
...
if hyper_parameters["scheduler"] == "CAWR":
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=decay_steps, gamma=decay_rate) # 梯度衰减策略
