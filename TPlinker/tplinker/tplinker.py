...
class HandshakingTaggingScheme(object):
    ...
    def __init__(self, rel2id, max_seq_len):
        ...
        self.shaking_ind2matrix_ind = [(ind, end_ind) for ind in range(self.matrix_size) for end_ind in list(range(self.matrix_size))[ind:]] #上三角矩阵
    ...
    def get_spots(self, sample):
        ...
        return ent_matrix_spots, head_rel_matrix_spots, tail_rel_matrix_spots # 得到三类矩阵，将数据集中的数据用 头-尾 头-头 尾-尾 表示
    ...
    def sharing_spots2shaking_tag(self, spots):
        ...
        return shaking_seq_tag # 生成的batch_shaking_seq_tag维度就是[batch,5+4+3+2+1]，即优化到上三角矩阵
    ...
    def sharing_spots2shaking_tag4batch(self, batch_spots):
        ...
        '''
        shaking_seq_len = self.matrix_size * (self.matrix_size + 1) // 2 # 生成n*n/2的字符对列排，两两配对
        batch_shaking_seq_tag = torch.zeros(len(batch_spots), shaking_seq_len).long()
        for batch_id, spots in enumerate(batch_spots): # 对这个序列进行序列标注
            for sp in spots:
                shaking_ind = self.matrix_ind2shaking_ind[sp[0]][sp[1]]
                tag_id = sp[2]
...
class TPLinkerBert(nn.Module):
    def __init__(self, encoder, 
        ...
        self.ent_fc = nn.Linear(hidden_size, 2) # 实体预测最后两个维度，两种标签0和1，是或者不是
        self.head_rel_fc_list = [nn.Linear(hidden_size, 3) for _ in range(rel_size)] # 初始化是一个每一关系中的实体头矩阵
        self.tail_rel_fc_list = [nn.Linear(hidden_size, 3) for _ in range(rel_size)] # 初始化是一个每一关系中的实体尾矩阵
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        shaking_hiddens = self.handshaking_kernel(last_hidden_state) #bert编码后的矩阵通过公共编码优化成上三角矩阵
...
class TPLinkerBiLSTM(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        # shaking_hiddens: (batch_size, 1 + ... + seq_len, hidden_size)
        shaking_hiddens = self.handshaking_kernel(lstm_outputs) # 将层拼接起来
        shaking_hiddens4ent = shaking_hiddens
        shaking_hiddens4rel = shaking_hiddens
        ...
        if self.dist_emb_size != -1:
            ...
            if self.dist_embbedings is None:
                ...
                for d in range(self.dist_emb_size):
                    for i in range(hidden_size):
                        if i % 2 == 0:
                            dist_emb[d][i] = math.sin(d / 10000**(i / hidden_size)) # 添加位置编码
                        else:
                            dist_emb[d][i] = math.cos(d / 10000**((i - 1) / hidden_size))
                seq_len = input_ids.size()[1]
                dist_embbeding_segs = []
...
class MetricsCalculator():
    ...
    def get_sample_accuracy(self, pred, truth):
        ...
        # seq维上所有tag必须正确，所以correct_tag_num必须等于seq的长度才算一个correct的sample，torch.ones_like返回一个大小同correct_tag_num的数值全为1的矩阵
